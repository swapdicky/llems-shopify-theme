...
(Catalog, Collaboration, About LLEMS, Cart) and a 4-column product grid. The
footer is rendered by a separate `llems-footer` section: a tagline plus a
4-column link layout. The global Dawn footer group is no longer required for
the homepage but remains available to other routes. The legacy
